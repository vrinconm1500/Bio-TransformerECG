...
# load dataset
pickleIn = open('datasets/CYBHi_short-1500.pickle', 'rb')
...
def step_decay(epoch):
    x=0.0039
    decay_rate =0.5
    if epoch >=40: x=x*decay_rate
    if epoch >=80: x=x*decay_rate
...
archivo_modelo = 'modelos/CYBHi-short_1500.h5'
...
df.to_csv('CYBHi-short-training/historico.csv', index=False)
...
plt.xlabel('epoch')
plt.savefig("CYBHi-short-training/Training and Validation Loss  ACC.jpg", dpi = 600)
plt.show()
...
plt.savefig("CYBHi-short-training/cm.jpg", dpi = 600)
